...
# What I shipped (Roger)

See [DECISIONS.md](./DECISIONS.md) for the full rationale. Summary:

- **Dashboard `/`** is built around *change*: the live Monday number (connected calls, last 7 days), a **"Talk to these on Monday"** list (biggest week-over-week drops), a **"Trending up"** list, and a full agent roster with WoW deltas. Mobile-first (works at 375px).
- **All SQL lives in `src/lib/db.ts`**; the page and four API routes are thin callers. Every window is a live rolling window from `datetime('now')`, so numbers are correct on any dataset.
- **API:** `/api/weekly-digest`, `/api/weekly-digest.csv`, `/api/agents/[id]/scorecard`, `/api/teams/[name]/summary` — live-queried, `no-store`, 404s for unknown agent/team, RFC-4180 CSV escaping.
- **Tests:** `pnpm test` (6 passing) re-derive each metric independently and assert the data layer matches — verifiable on any dataset.
